    ...
    for col in ["Waste Collected (Kg)", "Before Weight", "After Weight (Kg)"]:
        if col in vtcs_df.columns:
            vtcs_df[col] = pd.to_numeric(
                vtcs_df[col].astype(str).str.replace(",", "", regex=False),
                errors="coerce",
            )
...
def find_zone_in_precheck_window(track_df, check_time, geo_df, window_minutes=60):
    ...
    if not {"Time", "Latitude", "Longitude"}.issubset(track_df.columns):
        return "Unknown"
    if not {"Name", "Latitude", "Longitude"}.issubset(geo_df.columns):
        return "Unknown"

    start_time = check_time - timedelta(minutes=window_minutes)
    end_time = check_time
    window_pings = track_df[
        (track_df["Time"] >= start_time) & (track_df["Time"] <= end_time)
    ].copy()
    ...
    for _, ping in window_pings.iterrows():
        for _, loc in geo_df.iterrows():
            if pd.isna(loc["Latitude"]) or pd.isna(loc["Longitude"]):
                continue
            radius = loc.get("Radius_Meters", 150)
            distance = haversine(
                ping["Latitude"], ping["Longitude"], loc["Latitude"], loc["Longitude"]
            )
            if distance <= radius:
                return f"✅ {loc['Name']}"
...
def build_column_config(df, min_px=90, max_px=280):
    ...
    for col in df.columns:
        ...
        width_label = "small" if width_px <= 110 else "medium" if width_px <= 180 else "large"
    ...
    if tracking_files:
        for uploaded_file in tracking_files:
            try:
                ...
                tracking_files_map[normalize_name(uploaded_file.name)] = temp_df
...
def detect_office_label(df):
    if df is None or "Office" not in df.columns:
        return "—"
    office_vals = df["Office"].dropna().astype(str).str.strip()
...
def process_audit(vtcs_df, tracking_files_map=None, geo_data=None):
    ...
    if "Vehicle" not in vtcs_df.columns:
        vtcs_df["Vehicle"] = "Unknown Vehicle"
    if "Data ID" not in vtcs_df.columns:
        vtcs_df["Data ID"] = range(1, len(vtcs_df) + 1)
    ...
    for _, row in vtcs_df.iterrows():
        ...
        elif "Status" in gps_pings.columns:
            stts = gps_pings["Status"].astype(str).str.lower().values
            valid_idle = any(keyword in s for s in stts for keyword in ["idle", "parked", "stopped"])
            gps_audit.append("✅ Verified" if valid_idle else "❌ Moving")
        else:
            gps_audit.append("❓ Status Missing")

        if geo_data is not None:
            z_found = find_zone_in_precheck_window(track_df, t_time, geo_data, 60)
        else:
            z_found = "Unknown"
...
def build_exec_summary(daily_metrics=None, monthly_metrics=None):
    ...
    if daily_metrics:
        messages.append(
            f"Daily VTCS review processed <b>{daily_metrics['trip_count']}</b> trips across <b>{daily_metrics['active_vehicles']}</b> active vehicles with <b>{daily_metrics['total_tonnage']:.1f} tons</b> recorded."
        )
        messages.append(
...
def render_module(module_title, module_subtitle, results, metrics):
    ...
    with c1:
        ...
        tons_fig.update_layout(
            ...
            height=420,
            xaxis=dict(title="Vehicle", showgrid=False),
            yaxis=dict(title="Tons", gridcolor="#e3eee4", zerolinecolor="#d3e4d5"),
            showlegend=False,
        )
    ...
    with c2:
        ...
        trips_fig.update_layout(
            ...
            height=420,
            xaxis=dict(title="Vehicle", showgrid=False),
            yaxis=dict(title="Trips", gridcolor="#e3eee4", zerolinecolor="#d3e4d5"),
            showlegend=False,
        )
    ...
    with tabs[0]:
        st.markdown(
            ...
            <div class="section-card">
                ...
                <div class="section-subtitle">Management-ready summary with trips, tonnage, and average cycle time by vehicle.</div>
            </div>
            """,
            unsafe_allow_html=True,
        )

        summ = results.groupby("Vehicle").agg({"Tonnage": "sum", "Data ID": "count", "Duration_Mins": "mean"}).rename(
            columns={"Data ID": "Trips", "Tonnage": "Total Tons", "Duration_Mins": "Avg Mins"}
        )
        summary_df = summ.reset_index().copy()
        st.dataframe(
            summary_df.style.background_gradient(cmap="Greens", subset=["Total Tons"]).format(
                {"Total Tons": "{:.2f}", "Avg Mins": "{:.1f}"}
            ),
    ...
    with tabs[1]:
        st.markdown(
            """
            <div class="section-card">
                <div class="section-title">Technical Audit Log</div>
                <div class="section-subtitle">Complete operational log with tracker matching, GPS audit result, and zone validation.</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        for extra_col in ["Tracking_File_Match", "GPS_Audit", "Zone_Check"]:
            if extra_col in results.columns:
...
# =========================================================
logo_html = (
    f'<img src="data:image/png;base64,{LOGO_BASE64}" alt="SGWMC Logo">'
    if LOGO_BASE64
    else '<div style="font-size:40px;">♻️</div>'
)
...
with st.sidebar:
    if LOGO_BASE64:
        st.markdown(
            f'<div style="display:flex;justify-content:center;margin-bottom:12px;"><img src="data:image/png;base64,{LOGO_BASE64}" width="92" style="border-radius:18px;background:white;padding:6px;"></div>',
            unsafe_allow_html=True,
        )
    else:
    ...
    with st.expander("TCP & WE Settings", expanded=True):
        ...
        if geo_upload:
            try:
                raw_geo_df = pd.read_excel(geo_upload) if geo_upload.name.lower().endswith("xlsx") else pd.read_csv(geo_upload)
